...
```bash
# リポジトリをクローン
git clone git@github.com:fcafe/mcp-search-console.git
...
```json
{
  "mcpServers": {
    "search-console": {
      "command": "node",
      "args": ["/path/to/mcp-search-console/dist/index.js"],
      "env": {
...
`/path/to/mcp-search-console` は実際のパスに置き換えてください。
...
- 再認証が必要な場合は `npm run auth` を再度実行してください
