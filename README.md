...
### 1. GitHub Packages の設定

`~/.npmrc` に以下を追加（GitHub のアクセストークンが必要です）:

```
@fcafe:registry=https://npm.pkg.github.com
//npm.pkg.github.com/:_authToken=YOUR_GITHUB_TOKEN
```

> **トークンの作り方**: GitHub → Settings → Developer settings → Personal access tokens → Generate new token → `read:packages` にチェック

### 2. インストール

```bash
npm install -g @fcafe/mcp-search-console
```
...
# ブラウザが開いて Google 認証が走ります
mcp-search-console auth
```
...
- 再認証が必要な場合は、ステップ4のコマンドを再度実行してください

## アップデート

```bash
npm update -g @fcafe/mcp-search-console
```
